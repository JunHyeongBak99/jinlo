...
def on_change():
    ...
    if user_input:
        ...
        if openai.api_key:
            ...
            st.session_state["chat"].append(("🙋", now, user_input))
            ...
            now = datetime.now(local_tz).strftime("%H:%M")  # Get the current time in the local time zone
            st.session_state["chat"].append(("🙇", now, response))
...
if not st.session_state.get("initial_question_displayed", False):
    initial_question = st.session_state.chatbot.get_initial_question()
    st.session_state["chat"].append(("🙇", datetime.now(local_tz).strftime("%H:%M"), initial_question))
    st.session_state["initial_question_displayed"] = True  # 초기 질문 출력 후에 상태를 True로 설정

# 대화 히스토리를 저장하는 함수
def save_conversation():
    # 대화 히스토리 저장 전에 "messages" 키 초기화
    ...
    st.session_state["chat"].append(("🙇", now, "상담을 종료합니다."))
    st.session_state["initial_question_displayed"] = False

    # 이 부분에서 "messages" 키 초기화
    st.session_state["messages"] = [{"role": "system", "content": "You are a has personality assistant. Respond to all input in 25 words and answer in korea"}]
...
with col1:
    ...
    for sender, time, message in st.session_state.get("chat", []):
        if sender == "🙋":
            color = "black"
            st.markdown(f'<div style="color:{color};font-size:16px;">🙋익명 {time}</div><div style="background-color:#f0f0f0;color:black;border-radius:12px;padding:10px 10px;margin:10px 0;float:left;">{message}</div>', unsafe_allow_html=True)
        else:
            color = "black"
            st.markdown(f'<div style="color:{color};font-size:16px;text-align:right;">🙇진로상담사 {time}</div><div style="background-color:#ffcccb;color:black;border-radius:12px;padding:10px 10px;margin:10px 0; float:right; width:98%; height:50%;">{message}</div>', unsafe_allow_html=True)
...
if not openai.api_key:
    ...
    if st.button("질문"):
        if question:
            # 채팅을 시각화하기 위해 질문 내용 저장
            now = datetime.now(local_tz).strftime("%H:%M")  # Get the current time in the local time zone
            st.session_state["chat"].append(("🙋", now, question))
            # GPT 모델에 넣을 프롬프트를 위해 질문 내용 저장
